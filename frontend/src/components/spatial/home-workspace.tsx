...
import { clientAuth, isGuestUser, isGoogleAuthEnabled, openGoogleWorkspace, saveAccessWithGoogle } from "@/lib/firebase";
...
export function HomeWorkspace() {
  ...
  const googleAuthEnabled = session.ready && isGoogleAuthEnabled();
  ...
  return <div className="home-product"><a className="skip-link" href="#home-main">Skip to homes</a>
    <header className="home-header"><HomeBrand /><nav aria-label="Workspace"><Link href="/join">Join a room</Link>{googleAuthEnabled && isGuestUser(session.user) && visibleHomes.length > 0 && <Button variant="ghost" disabled={busy} onClick={() => void google()}>Keep access with Google</Button>}</nav></header>
    <main id="home-main" className="home-library"><div className="home-library-heading"><div><h1>Your homes</h1><p>A space to imagine, refine, and agree on a home.</p></div><div className="home-library-actions"><Button disabled={busy || !session.user} onClick={() => void create()}><Plus size={18} aria-hidden="true" />{busy ? "Creating project…" : "New project"}</Button></div></div>
      {(error || session.error) && <div className="home-error" role="alert"><p>{error || session.error}</p><Button variant="outline" disabled={session.connecting} onClick={() => { if (!session.user) void session.reconnect(); else setRefresh(value => value + 1); }}>{session.connecting ? "Connecting…" : session.user ? "Reconnect" : session.actionLabel}</Button></div>}
      ...
      : <section className="home-first-use"><div><h2>Your next home starts with a possibility.</h2><p>Choose a complete floor plan. Tell your design assistant what matters. See furniture, finishes and lights come together in a home you can explore.</p><div className="home-library-actions"><Button disabled={busy || session.connecting || !session.ready} onClick={() => { if (session.user) void create(); else void session.reconnect(); }}>{session.connecting ? "Connecting…" : session.user ? "Create your first home" : session.actionLabel} <ArrowRight size={18} aria-hidden="true" /></Button><Button asChild variant="outline"><a href="/demo/index.html"><Play size={18} aria-hidden="true" />Show tour</a></Button></div><p className="home-small">{session.accessMessage || "Your project saves privately as you work."}</p>{googleAuthEnabled && session.user && isGuestUser(session.user) && <Button variant="ghost" onClick={() => void google(true)} disabled={busy}>Open work saved with Google</Button>}</div><figure><Image src={homeTemplates[1].thumbnailPath} alt="An authored family-home layout with connected living spaces and two bedrooms" width={960} height={700} priority /><figcaption>A starting layout. Your design makes it yours.</figcaption></figure></section>}
      <footer className="home-library-footer"><p>{isGuestUser(session.user) ? "Your projects belong to this browser’s private guest workspace." : "Your saved projects are private until you invite a client."}</p>{googleAuthEnabled && isGuestUser(session.user) && visibleHomes.length === 0 && <Button variant="ghost" disabled={busy} onClick={() => void google()}>Keep access with Google</Button>}<Link href="/proposals">Scope & proposals</Link></footer>
    </main></div>;
}
