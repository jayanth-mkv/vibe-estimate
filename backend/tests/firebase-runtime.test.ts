...
const calls = vi.hoisted(() => ({ initialize: vi.fn(), adc: vi.fn(), adminFirestore: vi.fn(), directFirestore: vi.fn(), auth: vi.fn(), verify: vi.fn() }));
vi.mock("firebase-admin/app", () => ({ initializeApp: calls.initialize, applicationDefault: calls.adc }));
vi.mock("firebase-admin/auth", () => ({ getAuth: calls.auth }));
...
beforeEach(() => { vi.clearAllMocks(); vi.stubEnv("GOOGLE_CLOUD_QUOTA_PROJECT", "unrelated-inherited-project"); calls.initialize.mockReturnValue({ name: "synthetic-app" }); calls.auth.mockReturnValue({ verifyIdToken: calls.verify }); calls.verify.mockResolvedValue({ uid: "synthetic-user" }); });
...
describe("Firebase runtime routing", () => {
  ...
  it("initializes one production Firebase app and verifies only against its configured identity project", async () => {
    const projectId = "synthetic-production-project";
    const config = readConfig({
      APP_ENV: "production", NODE_ENV: "production", FIREBASE_PROJECT_ID: projectId,
      ...
      FIREBASE_WEB_CONFIG: JSON.stringify({ projectId, authMode: "google" }),
    });
    const credential = { type: "application-default" };
    calls.adc.mockReturnValue(credential);
    const runtime = createFirebaseRuntime(config);
    expect(calls.initialize).toHaveBeenCalledTimes(1);
    expect(calls.initialize).toHaveBeenCalledWith({ projectId, credential });
    expect(calls.auth).toHaveBeenCalledTimes(1);
    expect(calls.auth).toHaveBeenCalledWith({ name: "synthetic-app" });
    expect(calls.adminFirestore).toHaveBeenCalledTimes(1);
    expect(calls.adminFirestore).toHaveBeenCalledWith({ name: "synthetic-app" }, "(default)");
    expect(Object.keys(runtime).sort()).toEqual(["firestore", "verifyToken"]);
    await runtime.verifyToken("synthetic-production-token");
    expect(calls.verify).toHaveBeenCalledWith("synthetic-production-token", true);
    calls.verify.mockRejectedValueOnce(Object.assign(new Error("revoked"), { code: "auth/id-token-revoked" }));
    await expect(runtime.verifyToken("synthetic-revoked-token")).rejects.toMatchObject({ code: "auth/id-token-revoked" });
  });
});
