...
export function readConfig(env: NodeJS.ProcessEnv): AppConfig {
  ...
  if (geminiTransport === "vertex" && (appEnv === "production" || aiProvider !== "gemini")) throw new Error("Vertex transport is available only for explicitly configured local or connected Gemini reviews.");
