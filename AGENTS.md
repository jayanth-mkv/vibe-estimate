...
The user requests a simple, modern, accessible interface, a Next.js frontend and Express backend served together from one Google Cloud Run service, and a complete local testing setup. The deployed app's native GitHub delivery is documented in docs/deployment.md. Preserve frontend/ and backend/ as the application boundaries.
...
Cloud resources and hosting connections must be managed with Terraform. Do not create or modify infrastructure using ad hoc gcloud, Firebase CLI, or Console commands. Authorized pushes to GitHub main trigger Cloud Build checks, image building and the runtime Terraform apply. Cloud Run is the only application host; do not add a separate frontend hosting provider. Do not restore manual source-upload release scripts or use gcloud run deploy. Record planned, existing, imported, and created resources separately in docs/infrastructure-inventory.md.

Firebase identity, Firestore, event delivery and the Cloud Run backend use one explicitly configured application project. Verify the existing Cloud Build connection separately; its location can differ from the backend region. Existing resources must be discovered and imported into Terraform before management; do not replace them. Use only placeholders and emulator identifiers in committed examples.
