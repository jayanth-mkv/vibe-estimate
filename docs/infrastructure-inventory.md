...
| Removed | Unused destination session-signing role and runtime self-binding | Reviewed two-delete IAM plan applied; readback confirms both removed and shared IAM Credentials API remains enabled |
| Updated | Destination Auth domains for existing connected development | Separate one-update plan appended only `localhost`, preserving five production domains and all provider settings |
...
No remote Firebase-adoption state exists. A private local-state inventory found no adopted source database or rules; the old production local state is the already-backed-up serial from before GCS migration, not an active owner. That stale local copy was moved into a private archive after checking its lineage, serial and file hash; remote state and earlier backups are preserved. Read-only source inventory found the expected default Firestore database, web app and an undeployed Hosting site with zero releases; buckets, Pub/Sub topics/subscriptions, Eventarc triggers, BigQuery datasets and Runtime Config resources were empty. Cloud Asset inventory was unavailable because its API is disabled, so no comprehensive asset-inventory claim is made.

Google-only operation does not need runtime custom-token signing. A separate reviewed plan removed exactly the unused session-signing custom role and its self-binding, leaving the shared IAM Credentials API, Auth reader role, runtime, queue and event identities unchanged. The Terraform option defaults off and rejects enabling guest-session signing together with Google-only policy. Readback confirmed the role is deleted and the runtime binding is absent.

Destination Auth also retains the existing authorized connected-development workflow: a separate domain-only update appended `localhost`. Readback confirmed all six domains, Google-only providers and the unchanged pinned SDK payload. No IP address, port, OAuth credential or provider setting changed. The destination root's 16 offline mock checks passed, including explicit legacy-signing opt-in, its rejection under Google-only policy, and the loopback-domain boundary. A full refreshed destination foundation plan reported no changes after both cleanup applies; its state now manages 24 resources.
