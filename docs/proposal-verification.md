# Proposal workflow verification

Historical execution record for the original proposal and shared-room release, dated 5 September 2026. These results establish the tested behavior at that checkpoint. See [current verification](verification.md) and [home studio verification](v1-verification.md) for current release and Google-access status.
...
The configured GitHub repository returned HTTP 200 and public metadata through an unauthenticated API request. An 854-character description and demo-post draft with operator URLs were prepared in `../docs/private/submission-pack-2026-09-05T18-05-35-991Z.md`. The supplied challenge and submission-form records were re-read. A fresh read-only Cloud Run check confirmed the same image recorded below, ready status, campaign label, Gemini 3.7 Flash and unchanged shared ADC. The public event landing-page lookup could not be opened; the deadline remains a supplied snapshot value. No new Gemini call or application test suite was run for this documentation closeout. The focused public scan passed for 244 text files, 80 relative links across eight changed documents resolved, and whitespace checks passed. Google consent, the explicit Secret Manager Gemini API-key confirmation, remaining release work and actual demo publication/form submission retain their separate statuses.
