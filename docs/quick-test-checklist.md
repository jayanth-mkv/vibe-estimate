...
Production audit of `88f253f` on `feat/spatial-home-studio`, 5 September 2026: all 15 distinct browser checks have passing results across targeted runs, with seven real Gemini calls. The full [executed record](verification.md#production-journey-audit--5-september-2026) lists failures, fixes and evidence separately. Completed Google consent and the direct-review retry fix remain separate gates; future 3D releases are not implemented. Reset the statuses when repeating this checklist against another deployment.
...
| Fixture browser/API | [ui.spec.ts](../tests/e2e/ui.spec.ts), [rooms.spec.ts](../tests/e2e/rooms.spec.ts), [api.spec.ts](../tests/e2e/api.spec.ts): desktop/mobile journeys, validation, replay, failures, QR rotation and role denial. `rtk npm run test:e2e -- --timeout=90000` against the explicit fixture emulator stack only. | Pending |
| Supporting checks | `rtk npm test`; `rtk npm run test:config`; `rtk npm run test:auth --workspace @vibeestimate/frontend`; `rtk npm run test:rules`; `rtk npm run typecheck`; `rtk npm run lint --workspace @vibeestimate/frontend`; `rtk npm run build`; `rtk npm run check:public`. Rules require their documented isolated emulator setup. | Pending |
