# Archived plan detail (v2 + v3)

> Superseded by [v2.md](v2.md), the single future product plan. Preserved unchanged for the nine-role crew spec, A2A/IAM detail, evaluation gates, MCP toolsets and rate-card thinking. Nothing here is a current commitment.
...
Named for [v3](v2.backup.md#more-roles), not separate roles here: Translator/Tone, specialist Photo-reader, Follow-up. Surveyor and Stylist already support native images in v2.
...
Exposing the admitted asset catalog as an MCP server rather than a plain function tool makes it a swappable, versioned service — so a designer can later point at their own vendor catalog, and a furniture vendor can publish one we consume. That is the ecosystem hook. Rate-card, Maps and third-party vendor servers are [v3](v2.backup.md#more-mcp).
...
These extensions follow the required [v2 complete-product capabilities](v2.backup.md#complete-product-capabilities). Real website intake, native images, room-stop walkthroughs, spatial exports and the 12-plan benchmark are v2 requirements.
...
Multiple source pages and levels, explicit floor alignment and heights, stair links, cross-floor navigation. The current [V1 schema](v1.md#scene-catalog-and-gemini) has one populated `level`. Introducing multiple levels requires a new schema version and an explicit migration of saved scenes and historical references; it must preserve existing design agreements.
...
The real launcher, branded intake, origin restrictions and same-project studio handoff are required in [v2](v2.backup.md#complete-product-capabilities). v3 can add a richer programmatic host API and more appearance controls.
...
Extends the single Catalog MCP from [v2](v2.backup.md#mcp).
...
Room-stop walkthroughs and Play briefing are required in [v2](v2.backup.md#complete-product-capabilities). Add:
...
The complete **12-plan annotated benchmark** is a [v2 gate](v2.backup.md#complete-product-capabilities). Extend it here for multi-storey homes, new input types and supplier assets, with held-out cases. Keep synthetic tests separate from consented real-plan evidence.
