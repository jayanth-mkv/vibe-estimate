...
| `memberRole` / `requireDesigner` | `backend/src/room-domain.ts` | The room membership and role checks. Together with `assertOwner` these are the **only** two places authorization lives — extend them, never duplicate. |
...
| Memory | Vertex Agent Engine Memory Bank | Scoped per identity. |
| Voice | `gemini-3.1-flash-live-preview` over WebSocket | [v3](v3.md#voice) only. |

Pin exact versions. Check `frontend/node_modules/next/dist/docs/` before writing Next.js code — this version differs from training data, as `frontend/AGENTS.md` warns.
...
| Risk | Control |
| --- | --- |
| An agent hop carries the wrong tenant's context | Context bundles assembled server-side from the verified UID only; bundle hash recorded in `AgentTrace`; extraction cache keyed by owner, source hash, transform and prompt version. |
...
| Injection through a plan image, message, skill body or MCP response | All four are data. Admitted tool and asset IDs only, strict schemas, and no arbitrary URLs, code, storage paths, pricing or sharing operations exposed to any model. |
...
| The crew appears to speak for the designer | Nothing outbound without explicit approval. Handoff announced honestly. `AgentTrace` records the human outcome on every proposal. |
...
- **Agent isolation** — a bundle assembled for one tenant never contains another's content; a per-tenant skill never loads for the wrong tenant; caches keyed so a hit cannot leak; unauthenticated, stranger and client-role denial on every new route.
- **Skills and MCP** — injection rejected, `toolFilter` enforced, no write paths, and a cited rate still requiring designer confirmation.
