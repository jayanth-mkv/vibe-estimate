...
if (!baseURL || !evidence || !path.isAbsolute(evidence) || !fs.existsSync(evidence) || !isExternalEvidencePath(fs.realpathSync(path.resolve(__dirname, "../..")), fs.realpathSync(evidence))) throw new Error("Explicit target and external private evidence directory are required.");
